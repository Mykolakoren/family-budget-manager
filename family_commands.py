...
class FamilyCommands:
    """Команды для работы с семейным бюджетом"""

    def __init__(self, family_manager, user_manager):
        self.family = family_manager
        self.user_manager = user_manager

    async def family_setup_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if family_data:
            ...
            for member_id in family_data['members']:
                member_name = self.family.get_user_name(member_id)
...
• `/family_leave` - покинуть группу
    ...
    async def family_invite_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Команда /family_invite - пригласить в семью"""
        user_id = update.effective_user.id
        family_data = self.family.get_family_data(user_id)
...
✅ После присоединения вы будете видеть все семейные транзакции!
    ...
    async def family_stats_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Команда /family_stats - детальная семейная статистика"""
        user_id = update.effective_user.id
...
"""
    ...
    async def family_leave_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Команда /family_leave - покинуть семью"""
        user_id = update.effective_user.id
        family_data = self.family.get_family_data(user_id)
    ...
    async def handle_family_callbacks(self, update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str):
    ...
    def handle_family_id_input(self, user_id, family_id_text):
